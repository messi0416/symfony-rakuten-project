...
# プログラムID：batchsv04_deploy.sh
# プログラム名：本番反映用シェル
# BatchSV04のworkuserパスワードが必要

# 実行ユーザ。ホームディレクトリの指定に利用
EXEC_USER=`whoami`
DST_SERVER="BatchSV04"
...
RSYNC_OPT="${RSYNC_OPT} --include=/data/cron/crontab_batchsv04"
...
# Let's Encryptの更新やgitlabのバックアップ用なのでBatchSV04では不要
