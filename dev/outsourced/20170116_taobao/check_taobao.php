  ...
  private function _save_product($new_product) {
    ...
    foreach ($new_product['variants'] as $new_variant) {
      ...
      if ($old_variant) {
        ...
        $sql = "UPDATE `{$this->_table_prefix}variants` SET `in_stock` = :in_stock, `updated_at` = :updated_at WHERE `id` = :id";
        $stmt = $this->_db->prepare($sql);
        ...
        $sql = "INSERT INTO `{$this->_table_prefix}variants` (`product_id`, `uid`, `name`, `in_stock`, `created_at`, `updated_at`) VALUES (:product_id, :uid, :name, :in_stock, :created_at, :updated_at)";
        $stmt = $this->_db->prepare($sql);
        $stmt->bindParam('product_id', $product_id);
        $stmt->bindParam('uid', $new_variant['uid']);
...
$dsn = sprintf('mysql:host=%s;dbname=%s;charset=utf8;unix_socket=%s', $host, $dbname, $unix_socket);
