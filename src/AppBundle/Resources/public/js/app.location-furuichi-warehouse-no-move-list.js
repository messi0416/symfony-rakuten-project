 ...
 * 検索フォーム・一覧テーブル
 */
var vmLocationNoMoveList = new Vue({
  el: "#locationNoMoveList"
  , data: {
  ...
  , ready: function() {
    ...
    this.locationsNoMove = [];
    if (typeof(LOCATIONSNoMove) != 'undefined') {
      for (var i = 0; i < LOCATIONSNoMove.length; i++) {
        var row = LOCATIONSNoMove[i];
        row.checked = false;
        this.locationsNoMove.push(row);
      }
    }
  }
  , computed: {
    ...
    , checkedCode: function() {
      return this.locationsNoMove.slice().filter(function(row) {
        return row.checked;
      });
    }
  ...
  , methods: {
    ...
    , search: function() {
      ...
      if (addParams.length > 0) {
        url = url + '?' + addParams.join('&') + '&' + new Date().getTime() + '#tabLocationNoMoveList';
      }
    ...
    , toggleCheckAll: function() {
      ...
      for (var i = 0; i < this.locationsNoMove.length; i++) {

        this.locationsNoMove[i].checked = this.checkAll;
      }
    }
