...
var vmLocationWarehouseToPickingList = new Vue({
  ...
  , computed: {
    ...
    , listData: function() {
      ...
      list = list.filter(function(item, i) {
        ...
        // 絞込 ロケーション
        if (self.filterLocation.length) {
          var reg = new RegExp('^' + self.filterLocation, 'i');
          result = result && reg.test(item.locationCode);
        }
  ...
  , methods: {
    ...
    , submit: function(button) {
      ...
      // 移動先ロケーション入力チェック
      if (button == 'ok' && (! self.detailInfo.targetLocation)) {
        alert("移動先ロケーションが未入力です。");
        return;
      }
      ...
      var data = {
        ...
        , dst_location: self.detailInfo.targetLocation
