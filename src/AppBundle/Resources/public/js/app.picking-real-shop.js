 ...
 * 一覧ブロック
 */
var vmLocationWarehouseToPickingList = new Vue({
    el: "#locationWarehouseToPickingList"
  , data: {
  ...
  , mounted: function() {
    ...
    this.$nextTick(function() {
      self.dataUrl = $(self.$el).data('dataUrl');
      self.locationUrl = $(self.$el).data('locationUrl');
      self.submitUrl = $(self.$el).data('submitUrl');
      self.finishUrl = $(self.$el).data('finishUrl');
  ...
  , computed: {
    ...
    , listData: function() {
      ...
      list = list.filter(function(item, i) {
        ...
        // 絞込 ラベル種別
        if (self.filterLabelType.length > 0) {
          result = result && item.labelType == self.filterLabelType;
        }
...
var vmRecalculateForm = new Vue({
  ...
  , mounted: function() {
    const self = this;
    this.$nextTick(function() {
      self.recalculateUrl = $(self.$el).data('recalculateUrl');
      self.clearUrl = $(self.$el).data('clearUrl');
    })
  }
  , methods: {
    recalculate: function() {
      if (confirm("ピッキングリストを再作成します。\n\nよろしいですか？")) {
        window.location.href = this.recalculateUrl;
      }
    }
    , clear: function() {
      if (confirm("ピッキングリストを全て削除します。\n\nよろしいですか？")) {
