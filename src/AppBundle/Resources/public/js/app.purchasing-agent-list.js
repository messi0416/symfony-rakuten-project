 ...
 * メインブロック
 */
Vue.component('purchasing-agent-form-modal', {
    template: '#templatePurchasingAgentForm'
  , delimiters: ['(%', '%)']
  , props: [
...
var vmComponentPurchasingAgentListItem = {
    template: '#templatePurchasingAgentListTableRow'
  , props: [
     'item'
  ]
  , data: function() {
  ...
  , computed: {
    ...
    , plusnaoLoginUrl: function() {
      return this.plusnaoLoginUrlBase.replace(/__DUMMY__/g, this.item.loginName);
    }
  }
  , mounted: function() {
    this.$nextTick(function() {
...
// 一覧画面 一覧表
var vmPurchasingAgentList = new Vue({
    el: '#purchasingAgentList'
  , delimiters: ['(%', '%)']
  , data: {
      list: [] // データ
    , removeUrl: null
  ...
  , components: {
      'result-item': vmComponentPurchasingAgentListItem // 一覧テーブル
  }
  ...
  , mounted: function() {
    this.$nextTick(function () {
      ...
      if (PURCHASING_AGENT_LIST_DATA) {
        for (var i = 0; i < PURCHASING_AGENT_LIST_DATA.length; i++) {
          var item = PURCHASING_AGENT_LIST_DATA[i];
  ...
  , methods: {
    ...
    , removeItem: function (item) {
      ...
      if (!confirm('この依頼先とそのユーザ情報を削除してよろしいですか？')) {
        return;
      }
