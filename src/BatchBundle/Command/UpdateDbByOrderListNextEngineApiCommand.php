...
class UpdateDbByOrderListNextEngineApiCommand extends ContainerAwareCommand
{
  ...
  {
    $this
      ->setName('batch:update-db-by-order-list-next-engine-api')
      ->setDescription('受注明細取込（差分更新）処理')
  ...
  protected function execute(InputInterface $input, OutputInterface $output)
  {
    ...
    $logger->info('NEW 受注明細取込（差分更新）処理を開始しました。');
    ...
    try {
      ...
      $logExecTitle = sprintf('受注明細取込（差分更新）処理');
      $logger->setExecTitle($logExecTitle);
      $logger->initLogTimer();
      $logger->addDbLog($logger->makeDbLog(null, $logExecTitle, '開始', "モード[$mode]", "伝票番号[$voucherNumber]"));

      // 伝票番号指定なのに伝票番号が取得できない場合は終了
      // 本来はエラーであるべきだが、現在は画面実装の問題でそのようなデータが来てしまうので、正常終了とする
      if ($mode === 'target' && ! $voucherNumber) {
        $logger->addDbLog($logger->makeDbLog(null,  '伝票番号未指定のため、処理中止', '終了'));
        return 0;
      }
      ...
      $logger->info("受注明細差分更新　最終更新: " . $lastUpdated->format('Y-m-d H:i:s'));

      if(strpos ( $mode , 'past') !== false){
        $past_hour = preg_replace("/\D/",'',$mode);
        $lastUpdated->modify("-".$past_hour." hour");
        $logger->info("受注明細差分更新　変更後基準日時: " . $lastUpdated->format('Y-m-d H:i:s'));
      }
      ...
      if ($mode === 'target' && $voucherNumber) {
        $dataList = $this->searchReceiveOrder(
            self::SEARCH_TYPE_VOUCHER_NUMBER, self::SEARCH_DATA_TYPE_DETAIL, null, $voucherNumber, null, 0, $logger);
        ...
        $total += $this->searchAndUpdateDB(self::SEARCH_TYPE_ALLOCATE_DATE, self::SEARCH_DATA_TYPE_DETAIL, $lastUpdated, $dbMain, $logger); // 引当日ベース
      }
      $logger->info("受注明細差分更新：明細取得件数：$total");
...
EOD;
        ...
        if ($stmt->fetchColumn(0) > 0) {
          ...
          $logger->info('受注明細差分更新：　NextEngine店舗一覧更新');
        }
...
EOD;
        ...
        $neMallProcess->updateSalesDetailAnalyze('use_tmp', $importInfo);
...
//        $logger->info('(受注データ) メール処理補助用テーブル更新');
      ...
      if($mode === 'target' && $voucherNumber){
        $dataList = $this->searchReceiveOrder(
            self::SEARCH_TYPE_VOUCHER_NUMBER, self::SEARCH_DATA_TYPE_DELIVERY, null, $voucherNumber, null, 0, $logger);
        $total = count($dataList);
        if ($total) {
      ...
      $logger->info("受注明細差分更新：受注伝票　配送情報取得件数：$total");
      ...
      $logger->info('NEW 受注明細取込（差分更新）処理を完了しました。');
      ...
      $logger->error('受注明細取込（差分更新）処理 エラー:' . $e->getMessage() . ':' . $e->getTraceAsString());
      $logger->addDbLog(
          $logger->makeDbLog('受注明細取込（差分更新）処理 エラー', '受注明細取込（差分更新）処理 エラー', 'エラー終了')->setInformation($e->getMessage() . ':' . $e->getTraceAsString())
        , true, '受注明細取込（差分更新）処理 でエラーが発生しました。', 'error'
      );
   ...
   */
  private function searchReceiveOrder($searchType, $searchDataType, $targetDate, $targetVoucherNumber, $limit, $offset, $logger) {
    $query = array() ;
    ...
    if ($searchDataType === self::SEARCH_DATA_TYPE_DETAIL) {
      ...
      if ($searchType === self::SEARCH_TYPE_VOUCHER_NUMBER){ //
        ...
        $apiLabel = "API-Log 引当日";
        // 引当日検索は、引当日だけだと遅いので、受注状態が出荷確定済のものを除外する（50秒→30秒ぐらいに短縮できる）
        ...
        $query['receive_order_order_status_id-neq'] = TbSalesDetail::ORDER_STATUS_CODE_SHIPPING_FINISH;
      } else {
  ...
  private function searchAndUpdateDB($searchType, $searchDataType, $targetDate, $dbMain, $logger) {
    $limit = ($searchType === self::SEARCH_TYPE_UPDATE_DATE) ? 5000 : 500; // 一度に取得する上限
    ...
    do {
      $roopCount++; // 検索前に加算
      $dataList = $this->searchReceiveOrder($searchType, $searchDataType, $targetDate, null, $limit, $offset, $logger);
    ...
    } while ($count >= $limit && $roopCount < 100); // 1回の取得件数が、$limit 未満になるまで繰り返す。$roopCountによる制限は無限ループ避け。
    return $total;
  }
