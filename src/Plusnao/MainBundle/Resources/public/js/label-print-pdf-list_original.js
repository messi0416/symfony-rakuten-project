  ...
  Vue.component('pdf-label-items', {
    ...
    data: function () {
      return {
        ...
        , colName: this.item.supportColName // TODO displayRowNameへ差し替え
        , rowName: this.item.supportRowName // TODO displayRowNameへ差し替え
    ...
    , methods: {
      ...
      , reflectionRemainNum: function() {
        if (this.updateRemainNum != "" && Math.sign(this.updateRemainNum) == 1) {
          for (var i = 0; i < this.listData.length; i++) {
            this.listData[i].remainNum = this.updateRemainNum;
          }
        }
      }
    }
