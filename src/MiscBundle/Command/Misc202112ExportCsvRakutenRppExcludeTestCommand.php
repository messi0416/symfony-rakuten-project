 ...
 */
class Misc202112ExportCsvRakutenRppExcludeTestCommand extends ContainerAwareCommand
{
  ...
  {
    $this
    ->setName('misc:202112-export-csv-rakuten-rpp-exclude-test')
    ->setDescription('CSVエクスポート 楽天RPP除外テスト')
  ...
  protected function execute(InputInterface $input, OutputInterface $output)
  {
    ...
    $logger->info('楽天RPP除外ファイル出力処理（テスト）を開始しました。');
    ...
    $logExecTitle = sprintf('楽天RPP除外CSV出力（テスト）');
    ...
    try {
      ...
      $logger->info(print_r($this->uploadFiles, true));
      $logger->info('RPP（テスト）createNewFileまで完了');
      // $this->uploadFiles = ['/home/workuser/tmp/rpp-dummy.csv']; // テスト失敗用CSVパス
      // $this->uploadFiles = ['/home/workuser/tmp/sample_items_exclude.csv']; // テスト成功用CSVパス
      // $logger->info(print_r($this->uploadFiles, true));
  ...
  private function downloadCurrentFile($logger) {
    $logger->debug('楽天RPP除外ファイル出力処理（テスト） ダウンロード開始');
    $container = $this->getContainer();
    ...
    file_put_contents(
      $scrapingResponseDir . '/response-dlall-rpp-test.txt',
      $client->getResponse()
    );
    file_put_contents(
      $scrapingResponseDir . '/response-content-dlall-rpp-test.txt',
    ...
    $logger->info('楽天RPP除外ファイル出力処理（テスト） CSV取得成功。[' . $this->importFile . ']');
  }
  ...
  private function importCurrentFile($logger) {
    $logger->debug('楽天RPP除外ファイル出力処理（テスト） インポート開始：ファイル名=' . $this->importFile);
...
EOD;
  ...
  private function createNewFile($logger) {
    $logger->debug('楽天RPP除外ファイル出力処理（テスト） 生成開始');
...
EOD;
  ...
  private function uploadNewFile($logger) {
    ...
    foreach ($this->uploadFiles as $uploadFilePath) {
      ...
      try {
        ...
        $logger->info(print_r($resultInfo, true));
  ...
  private function uploadOneFile(UploadedFile $uploadFile, $xsrfToken) {
    ...
    $logger = $this->getLogger();
    $logger->info(print_r($csvUploadInfo, true));

    $resultInfo = ['status' => 'ok'];
    // 失敗
    if ($csvUploadInfo['status'] != 'SUCCESS') {
      $resultInfo['status'] = 'ng';
      $resultInfo['message'] = $csvUploadInfo['errors'];
      $resultInfo['detail'] = '';
      return $resultInfo;
    }
